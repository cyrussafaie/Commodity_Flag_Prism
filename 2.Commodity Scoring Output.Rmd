...
```{r}
final.desig=read.csv("data_for_score_calc_20160130.csv")
...
all=sqldf('select prod_nbr, count(distinct div_nm) as ttl_mrkt_itm
          ...
          where
          div_nm not in ("CHARLOTTE","MILWAUKEE-WAUKESHA") 
          group by prod_nbr ')
...
final_dataJan31=cbind(final_data_new_adj_majority,com_flg_final)
table(final_dataJan31$com_flg_final)

#write.csv(final_dataJan31,"final_dataJan31_1445.csv") #98% threshold        

final_dataJan31=read.csv("final_dataJan31_1445.csv")
dim(final_dataJan31)
#reapplying the exclusion filter
out2 <- ifelse(final_dataJan31$pim_cls_desc %in% pimclasses | final_dataJan31$pim_grp_desc %in% pimgroups | final_dataJan31$div_nm =="CHARLOTTE", "N",final_dataJan31$com_flg_final)
out2[out2 %in% 1] <- "N"
out2[out2 %in% 2] <- "Y"
table(out2)
final_dataJan31_final=cbind(final_dataJan31,com_flg_final_final=out2)
dim(final_dataJan31_final)

write.csv(final_dataJan31_final,"final_dataJan31_1525.csv",row.names = F)

final_dataJan31_final$ttl_mrkt_itm[is.na(final_dataJan31_final$ttl_mrkt_itm)]<-0

dim(final_dataJan31_final)
final_dataJan31_final$com_flg_final_final[final_dataJan31_final$ttl_mrkt_itm=1]<-final_dataJan31_final$com_flg_adj

unique(final_dataJan31_final$com_flg_adj)
final_dataJan31_final$ttl_mrkt_itm
...
```{r}
final_dataJan31_final=read.csv("final_dataJan31_1525.csv")
names(final_dataJan31_final)

write.csv(final_dataJan31_final,"final_dataJan31_1745.csv", row.names = F )
dim(final_dataJan31_final)
```


```{r}
table(reco=final_dataJan31_final$com_flg_final_final,current=final_dataJan31_final$cmdty_ind)
...
divs=sqldf('select div_nm, count(*) as "proposed" from final_dataJan31_final where com_flg_final_final="Y" and div_nm not in ("CHARLOTTE","MILWAUKEE-WAUKESHA") group by div_nm ')
...
div=sqldf('select div_nm, count(*) as "current" from final_dataJan31 where cmdty_ind="Y" and div_nm not in ("CHARLOTTE","MILWAUKEE-WAUKESHA") group by div_nm ')
...
```{r}
cls=sqldf('select pim_cls_desc, count(*) from final_dataJan31_final where  div_nm not in ("CHARLOTTE","MILWAUKEE-WAUKESHA") group by pim_cls_desc ')
cls[order(cls$'count(*)'),]
...
```{r}
cls=sqldf('select pim_cls_desc, count(*) from final_dataJan31_final where cmdty_ind="Y" and div_nm not in ("CHARLOTTE","MILWAUKEE-WAUKESHA") group by pim_cls_desc ')
